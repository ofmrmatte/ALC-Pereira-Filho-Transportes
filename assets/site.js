(() => {
    ...
    const buildMessage = data => {
      if (type === 'quote') {
        return lines(
          ...
          `Empresa: ${clean(data.empresa)}`,`Responsável: ${clean(data.nome)}`,`Telefone: ${clean(data.telefone)}`,`E-mail: ${clean(data.email)}`,`Serviço: ${clean(data.servico)}`,`Origem: ${clean(data.origem)}`,`Destino: ${clean(data.destino)}`,`Perfil da carga/operação: ${clean(data.carga)}`,`Frequência/volume: ${clean(data.volume)}`,data.mensagem ? `Observações: ${clean(data.mensagem)}` : ''
        );
      }
      if (type === 'career') {
        return lines('*TRABALHE CONOSCO — SITE ALC*','',`Nome: ${clean(data.nome)}`,`Cidade/UF: ${clean(data.cidade)}`,`Telefone: ${clean(data.telefone)}`,`Área de interesse: ${clean(data.area)}`,data.experiencia ? `Experiência: ${clean(data.experiencia)}` : '','','Gostaria de receber orientação sobre oportunidades e o canal correto para envio do currículo.');
      }
      return lines('*CONTATO — SITE ALC*','',`Nome: ${clean(data.nome)}`,`Telefone: ${clean(data.telefone)}`,`E-mail: ${clean(data.email)}`,`Assunto: ${clean(data.assunto)}`,'',clean(data.mensagem));
    };
  ...
  filterButtons.forEach(button => button.addEventListener('click', () => {
    ...
    people.forEach(person => { person.hidden = filter !== 'todos' && person.dataset.teamArea !== filter; });
  }));
